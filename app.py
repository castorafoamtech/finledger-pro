...
# ═══════════════════════════════════════════════════════════════
# ROUTES — Admin / One-time Migrations
# ═══════════════════════════════════════════════════════════════

@app.route('/api/admin/fix-remark-dates', methods=['GET', 'POST'])
def admin_fix_remark_dates():
    """
    One-time migration: for transactions with no dates, extract a date from
    the credit_remark or debit_remark field (Indian DD/MM or DD/MM/YY format)
    and assign it as both txn_date and transaction_date.
    GET  → preview (dry run, shows what would change, no DB writes)
    POST → apply changes
    """
    import re
    ...
    # Find all transactions with no dates at all
    undated = FinTransaction.query.filter(
        FinTransaction.txn_date.is_(None),
        FinTransaction.transaction_date.is_(None)
    ).all()
    ...
    matched, unmatched = [], []
    for t in undated:
        parsed = parse_date_from_text(t.credit_remark) or parse_date_from_text(t.debit_remark)
        remark = (t.credit_remark or t.debit_remark or '').strip()
        acct   = FinAccount.query.get(t.account_id)
        if parsed:
            if not dry_run:
                t.txn_date         = parsed
            ...
            matched.append({
                'id':      t.id,
                'account': acct.name if acct else str(t.account_id),
    ...
    return jsonify({
        'mode':        'preview (GET)' if dry_run else 'applied (POST)',
